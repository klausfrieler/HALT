...
prepare_item_bank <- function(){
  item_bank <- read.csv("data_raw/item_bank.csv", sep  =";", stringsAsFactors = F) %>%
    as_tibble()

  stimulus_map <- readxl::read_xlsx("data_raw/stimulus_mapping_v2.xlsx") %>%
    mutate(stimulus_id = tolower(id)) %>%
    select(-id)

  item_bank <- item_bank %>% left_join(stimulus_map, by = "stimulus_id")
  item_bank <- item_bank %>%
    pivot_longer(-c(page_id, stimulus_id, correct_answer), names_to = "type", values_to = "audio_file") %>%
    mutate(audio_file = Vectorize(utils::URLencode, SIMPLIFY = T)(audio_file, reserved = T) %>% as.vector())
  item_bank
}
item_bank <- prepare_item_bank()
